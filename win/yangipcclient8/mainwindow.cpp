    ...
    if(!m_isStartplay){
        ...
        if(m_player){
            if(!m_inited){
                ...
                m_player->setLoglevel(5);
                m_player->setMqttServer(m_isTls,m_mqttIP,m_mqttPort,NULL,NULL);
                m_player->setIceServer((char*)"192.168.0.104",3478,(char*)"metartc",(char*)"metartc");

                m_inited=true;
            }

            if(m_player->startMqtt((char*)"test1001")!=Yang_Ok)
                return;
